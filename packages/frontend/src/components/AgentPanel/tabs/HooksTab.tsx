...
export function HooksTab({ agentId: _agentId }: { agentId: string }) {
  ...
  useEffect(() => {
    fetch("/api/cli/run/hooks:list")
      .then((r) => r.json())
      .then((d: Hook[] | { items?: Hook[] }) => setHooks(Array.isArray(d) ? d : (d.items ?? [])))
      .catch(() => setHooks([]))
  ...
  const toggle = async (hook: Hook) => {
    ...
    try {
      const res = await fetch(`/api/cli/hooks/${action}/${encodeURIComponent(hook.name)}`, {
        method: "POST",
      });
      if (!res.ok) return;
      setHooks((hs) =>
        hs.map((h) => (h.name === hook.name ? { ...h, enabled: !h.enabled } : h))
      );
    } finally {
      setToggling(null);
    }
  ...
  return (
    <div className="p-3 space-y-2">
      {hooks.length === 0 ? (
        <p className="font-pixel text-xs text-gray-600">Aucun hook configuré</p>
      ) : (
        hooks.map((hook) => (
          <div key={hook.name} className="bg-pixel-bg border border-pixel-border p-2">
            <div className="flex justify-between items-center">
              <div className="min-w-0">
                <div className="font-pixel text-xs text-white">{hook.name}</div>
                {hook.description && (
                  <div className="font-pixel text-xs text-gray-500 mt-1">{hook.description}</div>
                )}
                {hook.events && (
